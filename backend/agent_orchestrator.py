...
class AgentOrchestrator:
    def __init__(self, session_id: str, provider: BaseProvider, model: str, cwd: str, mode: str = "default", context_window: int = 128000):
    ...
    def _get_allowed_tools(self) -> List[Dict[str, Any]]:
        mode_cfg = AGENT_MODES.get(self.mode, AGENT_MODES["default"])
        allowed_names = mode_cfg["allowed_tools"]
    ...
    async def run(self, prompt: str, system_prompt: Optional[str] = None) -> str:
        ...
        mode_cfg = AGENT_MODES.get(self.mode, AGENT_MODES["default"])
        base_system = system_prompt or mode_cfg["system_prompt"]

        for i in range(self.max_iterations):
            await self._manage_context()

            logger.info(f"Iteration {i+1} for session {self.session_id} (Mode: {self.mode})")
            ...
            for tool_call in tool_calls:
                ...
                # Double check tool permission
                if tool_name not in mode_cfg["allowed_tools"]:
                    result = f"Error: Tool {tool_name} is not allowed in {self.mode} mode."
                else:
    ...
    async def _summarize_context(self):
        ...
        resp = await self.provider.chat(model=self.model, messages=[{"role": "user", "content": summary_prompt}])
        if resp.status == 200:
    ...
    def _extract_assistant_message(self, body: Any) -> Dict[str, Any]:
        if "choices" in body and body["choices"]: return body["choices"][0]["message"]
        if "content" in body and isinstance(body["content"], list): return {"role": "assistant", "content": body["content"]}
            ...
            if "content" in cand:
                ...
                for p in parts:
                    ...
                    if "functionCall" in p:
                        fc = p["functionCall"]
                        tool_calls.append({"id": "gemini_call", "type": "function", "function": {"name": fc["name"], "arguments": json.dumps(fc["args"])}})
                msg = {"role": "assistant", "content": text}
                if tool_calls: msg["tool_calls"] = tool_calls
                return msg
        if "text" in body and "generation_id" in body: return {"role": "assistant", "content": body["text"]}
        return {"role": "assistant", "content": str(body)}

    def _extract_tool_calls(self, msg: Dict[str, Any]) -> List[Dict[str, Any]]:
        if "tool_calls" in msg and msg["tool_calls"]: return [{"id": tc["id"], "name": tc["function"]["name"], "arguments": json.loads(tc["function"]["arguments"])} for tc in msg["tool_calls"]]
        if isinstance(msg.get("content"), list):
            tool_calls = []
            for block in msg["content"]:
                if isinstance(block, dict) and block.get("type") == "tool_use": tool_calls.append({"id": block["id"], "name": block["name"], "arguments": block["input"]})
            return tool_calls
        return []
